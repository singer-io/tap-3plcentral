...
class ThreePLCentralDiscoveryTest(DiscoveryTest, ThreePLCentralBaseTest):
    ...
    @staticmethod
    def name():
        return "tap_tester_3plcentral_discovery_test"
    ...
    def test_parent_stream(self):
        """
        Test that each stream's metadata correctly includes the expected
        ...
        - Verifies that the metadata contains the `PARENT_TAP_STREAM_ID` key
          (except for orphan streams).
        ...
        for stream in self.streams_to_test():
            with self.subTest(stream=stream):
                ...
                actual_parent_tap_stream_id = (
                    stream_properties[0]
                    .get("metadata", {})
                    .get(self.PARENT_TAP_STREAM_ID, None)
                )
                ...
                # For streams with a parent, verify the parent tap stream ID
                ...
                # verify actual parent stream matches expected
